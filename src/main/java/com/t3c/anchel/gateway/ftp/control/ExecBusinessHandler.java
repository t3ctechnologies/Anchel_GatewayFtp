 ...
 */
package com.t3c.anchel.gateway.ftp.control;
...
import com.t3c.anchel.gateway.ftp.config.AUTHUPDATE;
import com.t3c.anchel.gateway.ftp.config.FileBasedConfiguration;
import com.t3c.anchel.gateway.ftp.database.DbConstant;
import com.t3c.anchel.gateway.ftp.file.FileBasedAuth;
import com.t3c.anchel.gateway.ftp.file.FileBasedDir;
import com.t3c.anchel.gateway.kernel.exec.AbstractExecutor;
import com.t3c.anchel.gateway.kernel.exec.R66PreparedTransferExecutor;
...
public class ExecBusinessHandler extends BusinessHandler {
	 ...
	 * Internal Logger
	 */
	private static final WaarpLogger logger = WaarpLoggerFactory.getLogger(ExecBusinessHandler.class);
	...
	@Override
	public void afterTransferDoneBeforeAnswer(FtpTransfer transfer) throws CommandAbstractException {
		// if Admin, do nothing
		...
		logger.debug(
				"Transfer done but action needed: " + (!(replyCode != ReplyCode.REPLY_250_REQUESTED_FILE_ACTION_OKAY
		...
		if (replyCode != ReplyCode.REPLY_250_REQUESTED_FILE_ACTION_OKAY
			...
			WaarpActionLogger.logErrorAction(dbFtpSession, specialId, transfer, message, getFtpSession().getReplyCode(),
					this);
			return;
		}
		...
		case RETR:
			...
			WaarpActionLogger.logAction(dbFtpSession, specialId, "Retrieve executed: OK", this,
		...
		case STOU:
			...
			try {
				...
				// File cannot be sent
				String message = "PostExecution in Error for Transfer since No File found: " + transfer.getCommand()
				...
				WaarpActionLogger.logErrorAction(dbFtpSession, specialId, transfer, message, exc.code, this);
				throw exc;
			}
			try {
				args[3] = file.getFile();
				...
				// Here the transfer is successful. If the file does not exist
				// on disk
				// We create it : the transfered file was empty.
				try {
					newfile.createNewFile();
				} catch (IOException e) {
					throw new Reply421Exception("PostExecution in Error for Transfer since No File found");
				...
				if (!newfile.canRead()) {
					// File cannot be sent
					String message = "PostExecution in Error for Transfer since File is not readable: "
							+ transfer.getCommand() + " " + newfile.getAbsolutePath() + ":" + newfile.canRead() + " "
							+ transfer.getStatus() + " " + transfer.getPath();
					CommandAbstractException exc = new Reply421Exception(
			...
			AbstractExecutor executor = AbstractExecutor.createAbstractExecutor(auth, args, true, futureCompletion);
			...
			if (futureCompletion.isSuccess()) {
				// All done
				WaarpActionLogger.logAction(dbFtpSession, specialId, "Post-Command executed: OK", this,
						getFtpSession().getReplyCode(), UpdatedInfo.RUNNING);
			} else {
				// File cannot be sent
				String message = "PostExecution in Error for Transfer: " + transfer.getCommand() + " "
						+ transfer.getStatus() + " " + transfer.getPath() + "\n   "
						+ (futureCompletion.getCause() != null ? futureCompletion.getCause().getMessage()
	...
	public void afterRunCommandKo(CommandAbstractException e) {
		...
		long specialId = ((FileBasedAuth) getFtpSession().getAuth()).getSpecialId();
		WaarpActionLogger.logErrorAction(dbFtpSession, specialId, null, message, e.code, this);
	...
	public void afterRunCommandOk() throws CommandAbstractException {
		if (!(this.getFtpSession().getCurrentCommand() instanceof QUIT) && this.dbR66Session != null) {
			long specialId = ((FileBasedAuth) getFtpSession().getAuth()).getSpecialId();
			WaarpActionLogger.logAction(dbFtpSession, specialId, "Transfer Command fully executed: OK", this,
					getFtpSession().getReplyCode(), UpdatedInfo.DONE);
			((FileBasedAuth) getFtpSession().getAuth()).setSpecialId(DbConstant.ILLEGALVALUE);
		}
	}
	...
	public void beforeRunCommand() throws CommandAbstractException {
		...
		// Test limits
		FtpConstraintLimitHandler constraints = ((FileBasedConfiguration) getFtpSession()
		...
		if (constraints != null) {
			...
			} else if (!FtpCommandCode.isSpecialCommand(getFtpSession().getCurrentCommand().getCode())) {
				...
				if (constraints.checkConstraintsSleep(1)) {
					if (constraints.checkConstraints()) {
						...
						if (FileBasedConfiguration.fileBasedConfiguration.ftpMib != null) {
							FileBasedConfiguration.fileBasedConfiguration.ftpMib.notifyOverloaded("Server overloaded",
									getFtpSession().toString());
						}
		...
		case STOU:
			...
			// create entry in log
			specialId = WaarpActionLogger.logCreate(dbFtpSession, "PrepareTransfer: OK",
		...
		case RETR:
			...
			specialId = WaarpActionLogger.logCreate(dbFtpSession, "PrepareTransfer: OK",
					getFtpSession().getCurrentCommand().getArg(), this);
			auth.setSpecialId(specialId);
			// execute the external retrieve command before the execution of
			// RETR
			WaarpFuture futureCompletion = new WaarpFuture(true);
			String[] args = new String[6];
			args[0] = auth.getUser();
			...
			args[5] = Long.toString(specialId);
			AbstractExecutor executor = AbstractExecutor.createAbstractExecutor(auth, args, false, futureCompletion);
			if (executor instanceof R66PreparedTransferExecutor) {
				((R66PreparedTransferExecutor) executor).setDbsession(dbR66Session);
			}
			executor.run();
			...
			if (futureCompletion.isSuccess()) {
				...
				if (!file.canRead()) {
					logger.error("PreExecution in Error for Transfer since "
							...
							+ (futureCompletion.getCause() != null ? futureCompletion.getCause().getMessage()
									: "File downloaded but not ready to be retrieved"),
							args[4], args[3]);
					throw new Reply421Exception("File downloaded but not ready to be retrieved");
				}
				WaarpActionLogger.logAction(dbFtpSession, specialId, "Pre-Command executed: OK", this,
						getFtpSession().getReplyCode(), UpdatedInfo.RUNNING);
			} else {
				...
				logger.error("PreExecution in Error for Transfer since "
						...
						+ (futureCompletion.getCause() != null ? futureCompletion.getCause().getMessage()
								: "File cannot be prepared to be retrieved"),
						args[4], args[3]);
				throw new Reply421Exception("File cannot be prepared to be retrieved");
			}
			break;
		default:
			// nothing to do
	...
	public void exceptionLocalCaught(Throwable cause) {
		if (FileBasedConfiguration.fileBasedConfiguration.ftpMib != null) {
			...
			FileBasedConfiguration.fileBasedConfiguration.ftpMib.notifyError("Exception trapped", mesg);
		}
	...
	public void executeChannelConnected(Channel channel) {
		if (AbstractExecutor.useDatabase) {
			if (com.t3c.anchel.openr66.database.DbConstant.admin != null
					&& com.t3c.anchel.openr66.database.DbConstant.admin.isActive()) {
				try {
					dbR66Session = new DbSession(com.t3c.anchel.openr66.database.DbConstant.admin, false);
				} catch (WaarpDatabaseNoConnectionException e1) {
					logger.warn("Database not ready due to {}", e1.getMessage());
					QUIT command = (QUIT) FtpCommandCode.getFromLine(getFtpSession(), FtpCommandCode.QUIT.name());
	...
	@Override
	public AbstractCommand getSpecializedSiteCommand(FtpSession session, String line) {
		if (getFtpSession() == null || getFtpSession().getAuth() == null) {
			return null;
		}
