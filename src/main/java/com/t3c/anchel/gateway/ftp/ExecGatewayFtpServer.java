 ...
 */
package com.t3c.anchel.gateway.ftp;
...
import com.t3c.anchel.gateway.ftp.service.FtpEngine;
 ...
 * standard Directory and FileInterface implementation (Filesystem based).
 * 
...
public class ExecGatewayFtpServer {
	...
	public static boolean initialize(String config, String r66file) {
		...
		if (AbstractExecutor.useDatabase) {
			// Use R66 module
			if (r66file != null) {
				if (!com.t3c.anchel.openr66.configuration.FileBasedConfiguration
						.setSubmitClientConfigurationFromXml(Configuration.configuration, r66file)) {
		...
		logger.warn("FTP started " + (configuration.getFtpInternalConfiguration().isUsingNativeSsl() ? "Implicit SSL On"
				: configuration.getFtpInternalConfiguration().isAcceptAuthProt() ? "Explicit SSL On" : "with SSL Off"));
		logger.info("*************************************************************");
