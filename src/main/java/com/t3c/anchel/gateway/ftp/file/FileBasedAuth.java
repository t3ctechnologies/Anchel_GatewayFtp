 ...
 */
package com.t3c.anchel.gateway.ftp.file;
...
import com.t3c.anchel.gateway.kernel.exec.AbstractExecutor.CommandExecutor;

/**
 * FtpAuth implementation based on a list of (user/password/account) stored in a
 * xml file load at startup from configuration.
 * 
...
public class FileBasedAuth extends FilesystemBasedFtpAuth implements HttpAuthInterface {
	 ...
	 * Internal Logger
	 */
	private static final WaarpLogger logger = WaarpLoggerFactory.getLogger(FileBasedAuth.class);
	...
	protected NextCommandReply setBusinessUser(String user) throws Reply421Exception, Reply530Exception {
		...
		return new NextCommandReply(FtpCommandCode.PASS, ReplyCode.REPLY_331_USER_NAME_OKAY_NEED_PASSWORD, null);
	}
	 ...
	 */
	@Override
	protected NextCommandReply setBusinessPassword(String password) throws Reply421Exception, Reply530Exception {
		if (currentAuth == null) {
			setIsIdentified(false);
			throw new Reply530Exception("PASS needs a USER first");
		}
		if (currentAuth.isPasswordValid(password)) {
			return new NextCommandReply(FtpCommandCode.ACCT, ReplyCode.REPLY_332_NEED_ACCOUNT_FOR_LOGIN, null);
		}
	 ...
	 */
	@Override
	protected NextCommandReply setBusinessAccount(String account) throws Reply421Exception, Reply530Exception {
		if (currentAuth == null) {
		...
		if (currentAuth.isAccountValid(account)) {
			...
			return new NextCommandReply(FtpCommandCode.NOOP, ReplyCode.REPLY_230_USER_LOGGED_IN, null);
		}
	...
	protected String setBusinessRootFromAuth() throws Reply421Exception {
		...
		if (account == null) {
			...
			path = FtpDir.SEPARATOR + user + FtpDir.SEPARATOR + account;
		}
	...
	public void specialNoSessionAuth(String hostid) {
		...
		((FtpSession) getSession()).setSpecialInit(this, new FileBasedDir(((FtpSession) getSession())),
